...
\caption{Comparison of the unconditioned (blue) and target-history conditioned (red) SURD synergy (left) and information leak (right) curves for the Core H\(\beta\) target, overlaid with the 95\% circular-shift surrogate confidence envelope (orange) for the conditioned scan. Conditioning on the target's own past shifts the synergy peak to 73 days, but this peak is statistically insignificant ($p \approx 0.98$) and is driven by finite-sample bias in the sparse 4D joint probability cells.}
