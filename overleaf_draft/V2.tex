...
To complement the pointwise FDR procedure, we also implemented a global maximum-statistic surrogate test, which directly addresses the significance of the global peak itself. For each of the 30 circular shift surrogate realizations, we compute the SURD scan over the full range of 120 lags and record the maximum synergy value ($S_{\mathrm{max, null}}$). This yields an empirical distribution of the peak synergy values expected purely by chance under the global null hypothesis. Comparing our real maximum synergy values ($0.228$ for Core, $0.339$ for Red, and $0.284$ for Blue) to this global max-statistic null distribution yields global p-values of $p_{\mathrm{global}} = 1.00$ (Core), $p_{\mathrm{global}} = 0.80$ (Red), and $p_{\mathrm{global}} = 0.97$ (Blue). This global test confirms that the observed synergy peaks cannot be rejected as random fluctuations under a global null procedure, supporting the cautionary conclusion that the synergy profiles represent broad, qualitative coupling envelopes rather than precise physical delays.
...
As shown in Figure~\ref{fig:synthetic_benchmarks}, SURD successfully recovers the true causal lags and information structures even under these realistic constraints. Over 15 independent Monte Carlo realizations, the quantitative performance metrics are summarized in Table~\ref{tab:synthetic_performance}.
...
Crucially, all cases show a systematic positive lag bias ranging from $+1.9$ to $+4.1$ days. This is a known estimator effect: in the presence of seasonal gaps, linear interpolation onto a 1-day grid acts as a low-pass filter, smoothing high-frequency variations and introducing a systematic phase delay (lag stretching). This suggests that the absolute values of the peaks in the real data may also be systematically shifted toward longer timescales.

\begin{table}[ht]
\centering
\begin{tabular}{lccccc}
\hline
\textbf{Case Model} & \textbf{True Lag (d)} & \textbf{Dominant Term} & \textbf{Mean Recovered Lag (d)} & \textbf{Recovery Bias (d)} & \textbf{False-Positive Rate} \\
\hline
Case 1: Single Driver & 15.0 & Unique ($U_1$) & $16.9 \pm 0.9$ & $+1.9$ & 0.93 \\
Case 2: Redundant Proxies & 15.0 & Redundancy ($R_{12}$) & $19.1 \pm 3.4$ & $+4.1$ & 1.00 \\
Case 3: Synergistic Drivers & 15.0 & Synergy ($S_{12}$) & $18.1 \pm 2.8$ & $+3.1$ & 0.93 \\
