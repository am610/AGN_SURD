...
To validate the mathematical behavior of the SURD algorithm under controlled physical scenarios, we construct a suite of four synthetic benchmark models. All driver signals are generated as Damped Random Walk (DRW) processes (representing the stochastic continuum variability of AGNs), and target signals are constructed with specific causal couplings:
\begin{enumerate}
    \item \textbf{Case 1: Single Driver.} The target responds to only one of the drivers with a delay of 15.0 days. As shown in Figure~\ref{fig:synthetic_benchmarks} (top-left), SURD isolates the unique information term $U_1$ peaking precisely at the true lag of 15.0 days, while synergy and redundancy remain at zero.
    \item \textbf{Case 2: Redundant Proxies.} The second driver is a noisy copy of the first. The target responds to the first driver with a 15.0-day delay. As shown in Figure~\ref{fig:synthetic_benchmarks} (top-right), the unique information drops to zero, and the redundancy term $R_{12}$ peaks at 15.0 days, showing how SURD correctly identifies redundant pathways.
    \item \textbf{Case 3: Synergistic Drivers.} The target depends on the non-linear product of two independent drivers delayed by 15.0 days. In Figure~\ref{fig:synthetic_benchmarks} (bottom-left), SURD isolates a clear synergy peak $S_{12}$ at 15.0 days, proving that it can detect non-linear joint dependencies.
    \item \textbf{Case 4: Two-Zone Response.} S1 is the prompt continuum, S2 is a delayed continuum (S1 delayed by 10.0 days), and the target is a weighted sum of $0.5 \times S1(t-10) + 0.5 \times S2(t-20)$. In Figure~\ref{fig:synthetic_benchmarks} (bottom-right), SURD resolves synergy peaks at the individual response delays of 10.0 and 20.0 days, demonstrating its capacity to decompose multi-zone geometries in the BLR.
\end{enumerate}

These benchmark cases show that the synergy and redundancy profiles generated by SURD reflect real physical causal structures and can be interpreted with confidence.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figure5_synthetic_benchmarks.png}
\caption{SURD information decomposition scans for the four synthetic benchmark cases: Case 1 (Single Driver), Case 2 (Redundant Proxies), Case 3 (Synergistic Drivers), and Case 4 (Two-Zone Response). Curves represent Synergy (purple), Redundancy (gray dashed), and Unique information for drivers S1 (blue dotted) and S2 (red dash-dot).}
