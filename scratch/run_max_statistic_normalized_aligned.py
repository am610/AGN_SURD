...
df = pd.read_csv('/Users/ayan/Programs/SURD/agn_surd_project/processed/strict_aligned_dataset.csv')
print("Loaded strict aligned dataset columns:", df.columns)

c_z = zscore(df['cont_flux'].values)
b_z = zscore(df['blue_wing_flux'].values)
y_z = zscore(df['core_flux'].values)
r_z = zscore(df['red_wing_flux'].values)
len_grid = len(df)
...
if __name__ == "__main__":
    print("Computing real max normalized synergies on aligned data...")
    ...
    print(f"Real max normalized synergies: Core={real_max_c:.6f}, Red={real_max_r:.6f}, Blue={real_max_b:.6f}")
    
    n_surrogates = 100
    print(f"Running {n_surrogates} circular shift normalized surrogates on aligned data...")
    with ProcessPoolExecutor() as executor:
        seeds = range(5000, 5000 + n_surrogates)
    ...
    print("\n--- ALIGNED GLOBAL NORMALIZED SIGNIFICANCE RESULTS (200 DAYS) ---")
